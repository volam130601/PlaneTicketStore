							...
							<table id="datatablesSimple">
								...
								<tbody>
									<c:forEach var="item" items="${ROUTE}">
										<tr>
											<td>${item.id}</td>
											<td><c:forEach var="temp" items="${AIRPORT}">
											...
											<td><span
												class="text-white p-1 fw-bold border-2 rounded 
												<c:if test="${item.status == 1}">bg-success</c:if>
												<c:if test="${item.status == 0}">bg-danger</c:if>">
													<c:if test="${item.status == 1}">Active</c:if> <c:if
														test="${item.status == 0}">Stopped</c:if>
											</span></td>
											<td>
											<button onclick="getID(${item.id})" class="btn btn-outline-warning"
